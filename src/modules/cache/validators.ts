...
export const validatorsCacheSet = async (validators: Validator[]) => {
    ...
    const validatorsKeyNameVal = filteredValidators.map((v) => ({key: v.name, val: v.publicKey}))
    const validatorsKeyPublicKeyVal = filteredValidators.map((v) => ({key: v.publicKey, val: v.name}))
